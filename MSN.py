...
class PyramidNet(nn.Module):
    def __init__(self, num_layers, alpha, block, num_classes=300):
        ...
        #수정
        self.conv1 = nn.Conv2d(in_channels=6, out_channels=16, kernel_size=3,
                               stride=1, padding=1, bias=False)
        ...
        self.fc_out = nn.Linear(3276, num_classes)
...
class MSN(NeuralNetwork):
    ...
    def __init__(self, word_embeddings, args):
        ...
        self.rnn = nn.GRU(
            input_size=200, hidden_size=256,
        ...
        self.emb_dim=200
        #수정
        self.Wc = nn.Parameter(torch.FloatTensor(2 * 256, 200))
        self.We = nn.Parameter(torch.FloatTensor(200, 200))

        block = ResidualBlock
        self.pyramid= PyramidNet(num_layers=6, alpha=90, block=block)#18 270
    ...
    def UR_Matching(self, bU_embedding, bR_embedding):
        ...
        Z = self.pyramid(M)
        V = self.tanh(Z)  # (bsz*max_utterances, 50)
        return V
    '''    
        M = self.get_Matching_Map(bU_embedding, bR_embedding)
    ...
    def get_weighted_key(self, key_r, key_mask_r):
        ...
        # sequence length
        s_len = key_r.size(1)
        key_emb = self.emb_drop(self.word_embedding(key_r.view(b_s * s_len, -1)))
    ...
    def forward(self, bU, bR, key_r,key_mask_r):
        '''
        ...
        #추가
        x1_emb = self.emb_drop(bU_embedding.reshape(bU_embedding.shape[0], -1, bU_embedding.shape[3]))
        _, c = self.rnn(x1_emb)
        ...
        # Equation 10
        b, s = bR.size(0), bR.size(1)
        alpha_k = F.softmax(
            torch.mm(c_k.view(b * s, -1), self.Wc).view(b, s, self.emb_dim) + torch.mm(key_emb_r.view(b * s, -1),self.We).view(b, s,self.emb_dim),dim=-1)
        # Equation 11
        bR_embedding = (1 - alpha_k) * bR_embedding + alpha_k * key_emb_r
