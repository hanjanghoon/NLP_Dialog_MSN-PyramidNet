...
class NeuralNetwork(nn.Module):
    ...
    def train_step(self, i, data):
        with torch.no_grad():
            batch_u, batch_r, batch_key_r, batch_key_mask_r,batch_y = (item.cuda(device=self.device) for item in data)
    ...
    def fit(self, X_train_utterances,  X_train_responses, y_train,
                  X_dev_utterances, X_dev_responses, y_dev,key_r,key_mask_r,dev_key_r,dev_key_mask_r):

        if torch.cuda.is_available(): self.cuda()

        dataset = DialogueDataset(X_train_utterances, X_train_responses,  key_r, key_mask_r ,y_train)
        ...
        for epoch in range(self.args.epochs):
            ...
            for i, data in enumerate(dataloader):
                ...
                if i > 0 and i % 1000000 == 0:
                    self.evaluate(X_dev_utterances, X_dev_responses,dev_key_r,dev_key_mask_r, y_dev)
                    self.train()
    ...
    def evaluate(self, X_dev_utterances, X_dev_responses, dev_key_r,dev_key_mask_r,y_dev, is_test=False):
        y_pred = self.predict(X_dev_utterances, X_dev_responses,dev_key_r,dev_key_mask_r)
    ...
    def predict(self, X_dev_utterances, X_dev_responses,dev_key_r,dev_key_mask_r):
        self.eval()
        y_pred = []
        dataset = DialogueDataset(X_dev_utterances, X_dev_responses,dev_key_r,dev_key_mask_r)
        dataloader = DataLoader(dataset, batch_size=400)

        for i, data in enumerate(dataloader):
            with torch.no_grad():
                batch_u, batch_r,batch_key_r,batch_key_masked_r = (item.cuda() for item in data)

                logits = self.forward(batch_u, batch_r,batch_key_r,batch_key_masked_r)
